...
from common import (iri_labels, iri_parents, iri_gates, synonym_iris, level_names, level_iris, iri_levels,
                    get_suffix_syns_symbs_maps, get_cell_iri_gates, split_gate)
  ...
  with open(args.output, 'w') as output:
    ...
    for row in rows:
      if not row['EXPERIMENT_ACCESSION'] in excluded_experiments:
        ...
        if row['CL term']:
          if preferized_gates:
            preferred_name = row['CL term'] + ' & ' + ', '.join(preferized_gates)
          else:
            preferred_name = row['CL term']
